...
f = snakemake.input
out_ca = snakemake.output.ca
out_sample = snakemake.output.sample
...
for ca in ca_list:
    ...
    c = token[0] # celltype
    a = token[1] # antibody
    m = df.loc[(df["antibody"] == a) & (df["celltype"] == c)]["mcc"].max() # Find the optimal MCC for each celltype-antibody combination
